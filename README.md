...
- **Quota**: the subscription used for `infra/` (`Visual Studio Enterprise Subscription – MPN`) currently has 0 TPM quota for `gpt-4.1` and `gpt-4o` in East US, and 0 quota for the `P1v3` App Service Plan. As a workaround, `openai.tf`'s `extraction` and `evaluation` deployments are temporarily pinned to `gpt-4o-mini`/`gpt-4.1-mini` (commented-out blocks show the original models) until quota is approved. The `querying` deployment stays on `gpt-4.1` since Azure AI Search's knowledge base query-planning model is restricted to the gpt-4o/gpt-4.1/gpt-5 families.
