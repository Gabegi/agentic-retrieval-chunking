...
public class PdfPigExtractor : IPdfExtractor
{
    ...
    // Exception types caught here are PdfPig's own (confirmed via reflection against
    // the referenced 0.1.9 build, not just docs) - anything else falls through to the
    // generic catch and is reported as Unknown rather than mislabeled as a specific cause.
