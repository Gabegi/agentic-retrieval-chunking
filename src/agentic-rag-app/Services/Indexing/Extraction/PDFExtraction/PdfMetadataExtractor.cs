...
namespace ProtocolsIndexer.Services
{
    ...
    internal static class PdfMetadataExtractor
    {
        ...
        // first-page text — there's no external index file for PDFs to join against,
        // unlike Zenya's index.csv. Shared by both IPdfExtractor backends so they parse
        // metadata identically regardless of which one is doing the text/table
        // extraction.
        //
        // These patterns are ported as-is from the PdfPig/Document Intelligence
        // comparison spike's LciMetadataParser (built against sample RIVM/LCI
        // infectious-disease guideline PDFs) — a working example, not a final ruleset.
        // Still worth keeping (both backends depend on it for Title/Version/
        // PublicationDateRaw today, so dropping it would blank those fields entirely),
        // but recalibrate once real Cordaan PDF filename/heading conventions are
        // confirmed - the Dutch-language, LCI-specific patterns below are very unlikely
        // to match Cordaan's own documents as-is.
