...
{
    // Everything read off an already-open PdfDocument via PdfPig, independent of Document
    // Intelligence entirely - the PDF's own Info-dictionary (Title/Author/CreationDate) and
    // its outline/bookmark tree. Both need the PdfDocument DocumentIntelligenceExtractor
    // opens during preflight, read inside the same `using` before it's disposed - neither
    // method opens or re-parses the file itself.
    internal static class PdfDocumentMetadataReader
    {
        // Native PDF Info-dictionary metadata (Title/Author/CreationDate) — distinct from
        // PdfMetadataExtraction.Parse, which derives Zenya's own Title/Version from the blob
        // name and first-page text.
        ...
        // fail it.
        public static IReadOnlyList<Bookmark>? GetBookmarks(PdfDocument pdf, string blobName, ILogger logger)
        {
            try
            {
