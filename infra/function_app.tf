...
# PHASE 1 of the subnet destroy/recreate (network.tf): the whole Function
# App is commented out here (not just virtual_network_subnet_id) so
# Terraform destroys it outright - that removes its AppServiceLink on
# cor-snet-cap-app-001 as a side effect, which is what actually frees the
# subnet for deletion (Terraform has no clean way to detach just the VNet
# integration in isolation - see prior attempts in git history). Its private
# endpoint and identity-scoped role assignments below are commented out too
# since they depend on this resource. PHASE 2: uncomment everything as-is -
# same config, virtual_network_subnet_id = azurerm_subnet.func.id will point
# at the new subnet once that's also back.
#
