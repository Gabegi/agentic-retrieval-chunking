...
# Search + the Foundry GPT deployment. Lives in its own RG
# (azurerm_resource_group.api, see resource_groups.tf) but shares the app
# subnet's outbound VNet integration with the indexing Function App (both
# delegate to Microsoft.Web/serverFarms). Private-endpoint-only inbound,
# matching the hub-firewall-routed architecture.
# ---------------------------------------------------------------------------
